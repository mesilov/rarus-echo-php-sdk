...
1. Сгенерированный CLI reference transcription-скилла. Перегенерируй его из текущего чекаута:
   ...
   Важно: если добавляешь новую команду или опцию, сначала внеси её во все нужные списки в `update-cli-reference.sh` — их три:
   - `PROJECT_COMMANDS` (bash-массив) — по нему генерируется help JSON команд;
   - `projectCommands` (node-массив) — по нему генератор обходит команды;
   - `optionAllowlist` (node) — разрешённые опции по каждой команде.

   Для новой команды нужны и `PROJECT_COMMANDS`, и `projectCommands`; для новой опции — соответствующая запись в `optionAllowlist`. Генератор и валидатор работают только по этим спискам, поэтому ключ или команда, не добавленный(ая) во все нужные списки, не попадёт в reference, а `make lint-agent-plugins` всё равно пройдёт — останется недокументированным без ошибки.
