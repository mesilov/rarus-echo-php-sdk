...
Доступны для любой команды:

| Опция | Значение | Описание |
| --- | --- | --- |
| `--json` | флаг | Вывести результат команды в формате JSON (опция приложения RARUS Echo). |
| `-h`, `--help` | флаг | Показать справку по команде. |
| `-q`, `--quiet` | флаг | Подавить вывод. |
| `-V`, `--version` | флаг | Показать версию приложения. |
| `-n`, `--no-interaction` | флаг | Не задавать интерактивных вопросов. |
| `-v`, `-vv`, `-vvv`, `--verbose` | флаг | Уровень детализации вывода. |
| `--ansi`, `--no-ansi` | флаг | Принудительно включить/выключить ANSI-раскраску. |

Кроме `--json`, глобальные ключи предоставляются Symfony Console.
