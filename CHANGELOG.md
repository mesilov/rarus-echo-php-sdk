...
### Added
- Parallel per-issue worktree tooling (`make worktree-new`, `worktree-remove`, `worktree-list` backed by `bin/worktree.sh`): creates a git-ignored `.worktree/<issue>-<slug>` checkout branched off `origin/<base>`, provisioned with a symlinked `.env.local` and a clone-copied `vendor/`, and removes it while keeping the branch.
